...
export function TimelineItem({
	...
	const statusConfig = {
		completed: {
			...
			iconNode: (
				<svg className="w-5 h-5 md:w-6 md:h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2} aria-hidden="true">
		...
		active: {
			...
			iconNode: (
				<svg className="w-5 h-5 md:w-6 md:h-6 fill-current" viewBox="0 0 24 24" aria-hidden="true">
		...
		upcoming: {
			...
			iconNode: (
				title?.toLowerCase().includes("phase 4") || title?.toLowerCase().includes("shortlisted") ? (
					<svg className="w-5 h-5 md:w-6 md:h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2} aria-hidden="true">
	...
	return (
		<div className={`relative flex flex-col md:flex-row md:justify-between group mb-16 md:mb-24 last:mb-0 ${className}`}>
			
			{/* Center Dot (Desktop) */}
			<div className={`hidden md:flex absolute left-1/2 -translate-x-1/2 top-0 items-center justify-center w-12 h-12 rounded-full z-10 transition-all duration-300 ${config.dotClass} ${config.iconColor}`}>
				{icon ? icon : config.iconNode}
			</div>

			{/* Center Dot (Mobile) */}
			<div className={`md:hidden absolute left-[28px] -translate-x-1/2 top-[4px] flex items-center justify-center w-10 h-10 rounded-full z-10 transition-all duration-300 ${config.dotClass} ${config.iconColor}`}>
				{icon ? icon : config.iconNode}
			</div>
			...
			<div className="w-full md:w-[calc(50%-3rem)] pl-[64px] md:pl-0 md:text-right pt-2">
				{date && (
					<div className={`text-[10px] md:text-xs font-bold uppercase tracking-widest mb-1.5 md:mb-2 ${config.dateClass}`}>
						{date}
						{status === "active" && <span className="text-primary-fixed"> • CURRENT PHASE</span>}
					</div>
				)}
				<h3 className={`text-xl md:text-2xl font-headline font-bold mb-3 md:mb-4 ${config.titleClass}`}>
					{title}
				</h3>
				...
				{rightContent && (
					<div className={`hidden md:block prose-sm sm:prose-base max-w-none md:ml-auto ${config.contentClass}`}>
						{children}
					</div>
				)}
			</div>
			...
			<div className="hidden md:block w-[calc(50%-3rem)] pt-2 text-left mt-4 md:mt-0">
				{rightContent ? rightContent : (
					<div className={`prose-sm sm:prose-base max-w-none ${config.contentClass}`}>
						{children}
					</div>
				)}
			</div>
